...
/*   Updated: 2026/03/02 22:31:31 by xzhen            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int ac, char **av)
{
	...
	if (!check_args(ac, av))//检查参数
		return (1);
	init_game_struct(&game);//初始化基础结构
	...
	if (!parse_file(av[1], &game))//解析地图文件
	{
		free_game(&game);
		return (1);
	}
	init_player_vectors(&game);//设置玩家初始向量
	if (!start_engine(&game))//启动图形引擎
	{
		free_game(&game);
		return (1);
	}
	mlx_loop(game.mlx_data->ptr_mlx);//进入消息循环
	return (0);
}
