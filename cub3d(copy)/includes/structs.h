...
/*   Updated: 2026/03/07 22:28:19 by xzhen            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
*/
# define BLOCK 64
# define WIN_WIDTH 1280 //窗口（屏幕）的宽度
# define WIN_HEIGHT 720 //窗口（屏幕）的高度
...
typedef struct s_texture
{
	...
	int		width;//贴图宽度
	int		height;//贴图高度
	int		line_len;//贴图一行多少字节
	int		endian;
	int		bpp;//每个像素多少 bit
}	t_texture;

/** ---------------------------------- DDA --------------------------------- **/
/* Data for the Digital Differential Analyser algorithm */ // DDA算法的工具箱

typedef struct s_dda
{
	float	ray_dir_x;//射线的方向
	float	ray_dir_y;
	int		map_x;//玩家/射线当前所在的地图格子,一定是整数
	int		map_y;
	float	side_dist_x;//玩家到第一条格子线的距离
	float	side_dist_y;
	float	delta_dist_x;//从一条竖线到下一条竖线的距离/射线跨过一个格子需要走多远
	float	delta_dist_y;//从一条横线到下一条横线的距离
	int		step_x;//射线在地图里走的方向
	int		step_y;
	int		side;//射线撞到的是哪种线,竖线还是横线
...
{
	void	*ptr_mlx;//MLX 系统的指针
	void	*window;//窗口指针
	void	*image;//图像缓冲区
	char	*image_add;//image 像素数据的内存地址
	int		bits_per_pixel;//每个像素占多少 bit
	int		line_length;//一行像素占多少字节
	int		endian;//字节序,通常不用管
}	t_mlx_data;
...
{
	t_player	player;// 玩家信息
	t_map		map;// 地图信息
	t_mlx_data	*mlx_data;// MiniLibX 核心数据（指针）
	t_texture	tex[4];// 四种纹理（数组）
	t_color		floor;// 地板颜色
	t_color		ceiling;// 天花板颜色
}	t_game;
